// app/api/plaid/historical/route.js
...
function mapCategory(primary, detailed, type) {
  ...
  if (d.includes('GAS') || d.includes('FUEL')) return 'Gas';
...
export async function POST(request) {
  try {
    ...
    // Plaid paginates historical transactions in batches of 500
    do {
...
const insert = db.prepare(`
    ...
    const insertMany = db.transaction(() => {
      for (const t of allTransactions) {
        ...
        const creditCategory = isCredit ? 'Income' : mapCategory(primary, detailed, t.transaction_type);
...
insert.run(
    ...
    return NextResponse.json({ success: true, total: allTransactions.length });
  } catch (error) {
    console.error('Historical sync error:', error.response?.data ?? error.message);
    return NextResponse.json({ error: 'Failed to pull historical data' }, { status: 500 });
  }
}